...
export async function fetchTokenBySymbol(baseUrl: string, symbol: string): Promise<Token | null> {
  const url =
    `${baseUrl}/api/tokens` +
    `?filters[symbol][$eq]=${encodeURIComponent(symbol.toUpperCase())}` +
    `&populate=logo`;

  const res = await fetch(url, { next: { revalidate: 60 } });
  if (!res.ok) throw new Error(`fetchTokenBySymbol failed: ${res.status} ${await safeReadText(res)}`);
  ...
  const logo = a?.logo?.data?.attributes?.url ?? a?.logo?.url ?? null;
