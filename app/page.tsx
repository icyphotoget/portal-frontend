...
export default async function HomePage() {
  ...
  const featuredRaw = articles.slice(0, 8);
  ...
  const latest = articles.slice(8, 18);
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      {/* subtle background */}
      <div className="absolute inset-x-0 top-0 h-[520px] bg-[radial-gradient(circle_at_20%_30%,rgba(255,255,255,0.08),transparent_55%)]" />
      <div className="absolute inset-x-0 top-0 h-[520px] bg-[radial-gradient(circle_at_80%_10%,rgba(255,255,255,0.06),transparent_55%)]" />

      <div className="relative mx-auto max-w-6xl px-4 pb-24 pt-6 sm:pt-8">
        {/* Top bar: centered brand */}
        <div className="grid grid-cols-3 items-center">
          <div className="flex items-center gap-2">
            {/* left: keep minimal (optional) */}
          </div>
          ...
          <div className="flex justify-end gap-2">
            <Link
              href="/news"
              className="rounded-xl border border-zinc-800 bg-zinc-900/30 px-4 py-2 text-sm text-zinc-200
                         transition hover:bg-zinc-900/55 hover:border-zinc-700"
            >
              News
            </Link>
            <Link
        ...
        {/* HERO = BIG FULL-WIDTH CAROUSEL */}
        <div className="relative mt-6 overflow-hidden rounded-[2rem] border border-zinc-800 bg-zinc-900/15 p-3 sm:p-4 lg:p-6 lg:shadow-[0_0_160px_rgba(255,255,255,0.06)]">
          <Glow />
          ...
          <div className="rounded-[1.6rem] overflow-hidden">
            {/* If your carousel has its own height controls, this container still makes it feel bigger */}
            <FeaturedCarousel items={featuredItems} intervalMs={5200} />
          </div>

          {/* optional: topics chips under carousel */}
          {categories.length > 0 ? (
            <div className="mt-5">
        ...
        {/* Footer (bez sitemap i “powered by…”) */}
        <footer className="mt-14 border-t border-zinc-800 pt-8 text-sm text-zinc-400">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="mt-1">Crypto news & on-chain narratives.</div>
            </div>
