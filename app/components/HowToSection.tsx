...
export default function HowToSection({
  ...
  seeAllHref = "/news",
  bg = "#F1B6D1", // tvoj pink
  accent = "#5B2BFF", // tvoj ljubičasti
  categorySlug = "how-to", // ✅ MATCH Strapi slug
  articles,
  takeItems = 4,
}: {
  label?: string;
  ...
  if (!howToArticles.length) return null;

  // 2) featured + items
  const hero = howToArticles[0];
  ...
  const items = howToArticles.slice(1, 1 + takeItems).map((a) => ({
    id: a.id,
    title: a.title,
    href: buildHref(a),
    author: pickAuthor(a),
        ...
        >
          ...
          <div className="relative flex items-center justify-between px-5 sm:px-6 lg:px-8 pt-5">
            <div className="flex items-center gap-3">
              <span className="h-5 w-[3px] rounded-full" style={{ background: accent }} />
